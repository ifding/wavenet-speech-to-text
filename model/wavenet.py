...
from model.networks import WaveNet as WaveNetModule
...
class WaveNet:
    ...
    def train(self, inputs, targets, input_percentages, target_sizes):
        """
        Train 1 time
        ...
        outputs = outputs.transpose(0, 1)  # TxNxH

        seq_length = outputs.size(0)
        sizes = Variable(input_percentages.mul_(int(seq_length)).int(), requires_grad=False) 

        loss = self.criterion(outputs, targets, sizes, target_sizes)   
